...
- Managed PostgreSQL enclave approval settings are applied during the initial enclave deployment. If `enclave.approvalSettings` is omitted, all four actions default to `NotRequired`. `Required` approvals must include at least one mandatory approver object ID and `minimumApproversRequired >= 1`; `NotRequired` approvals normalize to an empty approver list with minimum `0`.
